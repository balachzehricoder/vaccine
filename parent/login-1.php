...
if (isset($_POST["signup"])) {
  ...
  $PARENT_CNIC = mysqli_real_escape_string($conn, $_POST["PARENT_CNIC"]);
  ...
  if ($PARENT_PASSWORD !== $cpassword) {
    echo "<script>alert('Passwords do not match');</script>";
  } else {
    // Check if email already exists
    $check_email = mysqli_query($conn, "SELECT PARENT_EMAIL FROM parent WHERE PARENT_EMAIL = '$PARENT_EMAIL'");
    if (mysqli_num_rows($check_email) > 0) {
      echo "<script>alert('Email already exists');</script>";
    } else {
      // Insert new user into the database
      $hashed_password = password_hash($PARENT_PASSWORD, PASSWORD_DEFAULT); // Use password_hash() for secure password hashing
      $sql = "INSERT INTO parent (PARENT_NAME, PARENT_CNIC, PARENT_EMAIL, PARENT_PASSWORD) VALUES ('$PARENT_NAME', '$PARENT_CNIC', '$PARENT_EMAIL', '$hashed_password')";
...
if (isset($_POST["signin"])) {
  ...
  $check_email = mysqli_query($conn, "SELECT PARENTID, PARENT_PASSWORD, PARENT_NAME FROM parent WHERE PARENT_EMAIL = '$PARENT_EMAIL'");
  if (mysqli_num_rows($check_email) > 0) {
